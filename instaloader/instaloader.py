...
class Instaloader:
    ...
    def __init__(self,
        ...
        self.context = InstaloaderContext(sleep, quiet, user_agent, max_connection_attempts, bot)
    ...
    def download_pic(self, filename: str, url: str, mtime: datetime,
                     filename_suffix: Optional[str] = None, _attempt: int = 1, caption=None) -> bool:
        ...
        if self.commit_mode:
            ...
            if os.path.isfile(filename):
                self.context.log(filename + ' exists', end=' ', flush=True)
                return False
        self.context.get_and_write_raw(url, filename, caption=caption)
    ...
    def download_post(self, post: Post, target: Union[str, Path], caption=None) -> bool:
        """
                ...
                if posts:
                    ...
                    for post in profile.get_posts():
                        ...
                        with self.context.error_catcher("Download {} of {}".format(post, profile_name)):
                            ...
                            while True:
                                try:
                                    downloaded = self.download_post(post, target=profile_name, caption=post.caption)
                                    downloaded = True
                                    break
